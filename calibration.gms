...
IF(CARD(g) EQ 3,
    EP(g)       = 1 + 0.35*ORD(g) - 0.13*(ORD(g)**2);
    delta       = 0.60;
    );
...
Pens0(q,e,gr)	= PensR*Lab0(q,e)*EPQ("g2",q)*(1-Leis0("g2",q,e));
Pens0(q,e,gw)	= 0;
CTR0		= SUM((q,e),SUM(gr,PopQE0(q,e,gr)*Pens0(q,e,gr)))/SUM((q,e),SUM(gw,PopQE0(q,e,gw)*Lab0(q,e)*EPQ(gw,q)*(1-Leis0(gw,q,e))))
...
HBudg0Eq1(g+1,q,e)..
    ...
    =E=
    (1-WTxR0-CTR0)*Lab0(q,e)*EPQ(g,q)*(1-Leis0(g,q,e)) + ((Rint0v-KTxR0*(Rint0v-1))*B0v(g,q,e))$(ORD(g) GT 1) + Inh0v(g,q,e) - Beq0v(g,q,e) + Pens0(q,e,g)
    ;
...
HBudg0Eq2(gl,q,e)..
    ...
    =E=
    (1-WTxR0-CTR0)*Lab0(q,e)*EPQ(gl,q)*(1-Leis0(gl,q,e)) + (Rint0v-KTxR0*(Rint0v-1))*B0v(gl,q,e) + Inh0v(gl,q,e) - Beq0v(gl,q,e) + Pens0(q,e,gl)
    ;
...
GammaEq(g,q,e)..
    Gammav(g,q,e)
    =E=
    ((1-WTxR0-CTR0)*Lab0(q,e)*EPQ(g,q)) * ((Leis0(g,q,e)/Con0v(g,q,e))**(1/SigIntra))
    ;
...
VV0vEq(g,q,e)..
    VV0v(g,q,e)
    =E=
    (1+ (gammav(g,q,e)**SigIntra) * (((1-WTxR0-CTR0)*Lab0(q,e)*EPQ(g,q))**(1-SigIntra)))
...
A0(s)           =       Y0(s) / (Kdem0(s)**AlK(s) * Ldem0(s)**(1-AlK(s)));
...
SOLVE OLG1 MINIMIZING OBJ USING NLP;
ABORT$(OLG1.ModelStat GT 2)"Check Calibration: No Feasible Solution for ---> Model OLG1";
