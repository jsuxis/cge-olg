...
FlQual		Flag (1) for Quality Shock
;
...
EQUATIONS
    ...
    PensEq(q,e,t,g)	Size of the Pension
    GPensEq(t)		Financing of the Pension
...
QEq(s,t)        $(ORD(t) GT CARD(tp) AND ORD(t) LE CARD(tp)+CARD(tm))..
    LOG(Y(s,t))
    =E=
    LOG(A(s,t)) + AlK(s)*LOG(Kdem(s,t)) + (1-AlK(s))*LOG(Ldem(s,t))
    ;
...
LdemEq(s,t)     $(ORD(t) GT CARD(tp) AND ORD(t) LE CARD(tp)+CARD(tm))..
    ...
    =E=
    LOG((1-AlK(s))) + LOG(Y(s,t)) + LOG(P(s,t))
    ;
...
HBudgEq1(q,e,t+1,g+1)	$(ORD(t) GT CARD(tp) AND ORD(t) LT CARD(tp) + CARD(tm))..
    ...
    =E=
    (1-WTxR(t)-CTR(t))*wageE(e,q,t)*Lab(q,e,t)*EPQ(g,q)*(1-Leis(q,e,t,g)) + ((Rint(t)-KTxR(t)*(Rint(t)-1))*B(q,e,t,g))$(ORD(g) GT 1) + Inh(q,e,t,g) - Beq(q,e,t,g) + Pens(q,e,t,g)
    ;
...
HBudgEq2(q,e,t,gl)	$(ORD(t) GT CARD(tp) AND ORD(t) LE CARD(tp)+CARD(tm))..
    ...
    =E=
    (1-WTxR(t)-CTR(t))*wageE(e,q,t)*Lab(q,e,t)*EPQ(gl,q)*(1-Leis(q,e,t,gl)) + ((Rint(t)-KTxR(t)*(Rint(t)-1))*B(q,e,t,gl)) + Inh(q,e,t,gl) - Beq(q,e,t,gl) + Pens(q,e,t,gl)
    ;
...
InhEq(q,e,t,g)     $(ORD(t) GT CARD(tp) AND ORD(t) LE CARD(tp)+CARD(tm) AND InhR(g) NE 0)..
    ...
    =E=
    InhR(g)*SUM(gr, PopQE(q,e,t,gr)*Beq(q,e,t,gr))
    ;
...
LeisEq(q,e,t,g)	$(ORD(t) GT CARD(tp) AND ORD(t) LE CARD(tp)+CARD(tm))..
    ...
    =E=
    (Gamma(g,q,e)/((1-WTxR(t)-CTR(t))*wageE(e,q,t)*Lab(q,e,t)*EPQ(g,q) + mu(q,e,t,g)))**SigIntra
    ;
...
VVEq(q,e,t,g)	$(ORD(t) GT CARD(tp) AND ORD(t) LE CARD(tp)+CARD(tm))..
    VV(q,e,t,g)
    =E=
    (1+ (Gamma(g,q,e)**SigIntra)* (((1-WTxR(t)-CTR(t))*wageE(e,q,t)*Lab(q,e,t)*EPQ(g,q) + mu(q,e,t,g))**(1-SigIntra)))
...
GBudgEq1(t+1)	$(ORD(t) GT CARD(tp) AND ORD(t) LT CARD(tp)+CARD(tm))..
    ...
    =E=
    PGov(t)*Gov(t) + P("s3",t)*GovH(t) + P("s2",t)*GovE(t) + (Rint(t-1)*(PGov(t)/PGov(t-1)))*PGov(t-1)*Bond(t)
    ;
...
GBudgEqSS(t)	$(ORD(t) EQ CARD(tp)+CARD(tm))..
    ...
    + KTxR(t)*(Rint(t)-1)*B(q,e,t,g)))
    =E=
    PGov(t)*Gov(t) + P("s3",t)*GovH(t) + P("s2",t)*GovE(t) + Rint(t-1)*PGov(t)*Bond(t)
    ;
...
* Size of the Pension
PensEq(q,e,t,gr)	$(ORD(t) GT CARD(tp) AND ORD(t) LE CARD(tp)+CARD(tm))..
    Pens(q,e,t,gr)
    =E=
    PensR*wageE(e,q,t-1)*Lab(q,e,t-1)*EPQ("g2",q)*(1-Leis(q,e,t-1,"g2"))
    ;

* Financing of the Pension
GPensEq(t)		$(ORD(t) GT CARD(tp) AND ORD(t) LE CARD(tp)+CARD(tm))..
    CTR(t)
    =E=
    SUM((q,e),SUM(gr,PopQE(q,e,t,gr)*Pens(q,e,t,gr)))/SUM((q,e),SUM(gw,PopQE(q,e,t,gw)*wageE(e,q,t)*Lab(q,e,t)*EPQ(gw,q)*(1-Leis(q,e,t,gw))))
    ;
...
PEq(s,t)        $(ORD(t) GT CARD(tp) AND ORD(t) LE CARD(tp)+CARD(tm))..
    Y(s,t)
    =E=
    SUM((q,e,g), PopQE(q,e,t,g)*ConS(s,q,e,t,g)) + IS(s,t) + GovS(s,t) + GovH(t)$(ORD(s) EQ 2) + GovE(t)$(ORD(s) EQ 3)
    ;
...
Beq.L(q,e,t,g)   	=       Beq0(g,q,e);
Beq.FX(q,e,tp,g)	=	Beq0(g,q,e);
Beq.FX(q,e,t,g)$(ORD(g) LT CARD(g)) =       BeqR(g);
Inh.L(q,e,t,g)   	=       Inh0(g,q,e);
Inh.FX(q,e,tp,g)   	=       Inh0(g,q,e);
Inh.FX(q,e,t,gr)	=       InhR(gr);
...
WTxR.L(t)		=	WTxR0;
WTxR.FX(tp)		=	WTxR0;
Pens.L(q,e,t,g)		=	Pens0(q,e,g);
Pens.FX(q,e,tp,g)	= 	Pens0(q,e,g);
Pens.FX(q,e,t,gw)	=	0;
CTR.L(t)		=	CTR0;
CTR.FX(tp)		=	CTR0;
Obj.FX          	=       0;
...
IF(FlDemog1 EQ 1,
NN(q,e,t) $(ORD(t) GT CARD(tp) AND ORD(t) LE (CARD(tp)+2))
                                =       1.03*NN(q,e,t);
...
GovH(t)		=	SUM((q,e,g),PopQE(q,e,t,g)*HEAC.L(g));
GovE(t)		=	SUM((q,e,g),PopQE(q,e,t,g)*EDUC.L(g));
...
* Prductivity Shock
IF(FlProdu EQ 1,
    A(s,t)	$(ORD(t) GT CARD(tp)+1 AND ORD(t) LE CARD(tp)+5) = 1.01*A(s,t);
